    ...
    function addSignatures(uint8 v, bytes32 r, bytes32 s) public returns(bool) {
        for(uint i= 0 ;i<signers.length ;i++)
        {
            if(tx.origin == signers[i])
            {
                ...
                {
                     errorRepeatSignaturesEvent(evidence,evidenceId,v,r,s,tx.origin);
                     return false;
                }
            }
        }
       ...
       {
           errorAddSignaturesEvent(evidence,evidenceInfo,evidenceId,v,r,s,tx.origin);
           return false;
       }
    }
...
contract EvidenceSignersData{
        ...
        {
            Evidence evidence = new Evidence(evi, info, id, v, r, s, this, tx.origin);
